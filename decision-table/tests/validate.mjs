...
assert.ok(html.includes("理解度チェックに全問正解すると、リーダーへ完了通知が送られます"), "理解度チェック全問正解後にリーダーへ通知することを案内すること");
...
assert.ok(appSource.includes('mode: "cors"') && !appSource.includes('mode: "no-cors"'), "GASの応答を読めるCORS通信で通知すること");
assert.ok(appSource.includes("await response.json()") && appSource.includes("!response.ok || !payload.ok"), "通知リレーの成功・失敗を応答で判定すること");
assert.ok(/script\.google\.com\/macros/.test(dataSource), "公開データに通知リレーURLを設定すること");
assert.ok(appSource.includes("リーダーへの完了通知を送信しました。") && appSource.includes("通知を送信できませんでした。"), "通知の送信結果を画面に表示すること");
